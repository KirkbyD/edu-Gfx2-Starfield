...
bool cShaderManager::createProgramFromFile(std::string friendlyName, cShader& vertexShad, cShader& fragShader, bool replaceExisting /*=true*/)
{
	std::string errorText = "";
...
bool cShaderManager::createProgramFromFile(std::string friendlyName, cShader& vertexShad, cShader& fragShader, cShader& geomShader, bool replaceExisting /*=true*/)
{
	std::string errorText = "";
	...
	geomShader.ID = glCreateShader(GL_GEOMETRY_SHADER);
	geomShader.shaderType = cShader::GEOMETRY_SHADER;
	if (!this->m_loadSourceFromFile(geomShader))
		return false;

	if (!this->m_compileShaderFromSource(geomShader, errorText)) {
		this->m_lastError = errorText;
	...
	glAttachShader(curProgram.ID, vertexShad.ID);
	glAttachShader(curProgram.ID, fragShader.ID);
	glAttachShader(curProgram.ID, geomShader.ID);
	glLinkProgram(curProgram.ID);
	...
	glDetachShader(curProgram.ID, fragShader.ID);
	glDetachShader(curProgram.ID, vertexShad.ID);
	glDetachShader(curProgram.ID, geomShader.ID);
