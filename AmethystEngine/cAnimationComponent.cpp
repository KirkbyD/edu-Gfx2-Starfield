...
cAnimationComponent::cAnimationComponent(void)
	: pScene(nullptr), mNumBones(0), mGlobalInverseTransformation(glm::mat4(0.f)), AnimaTime(0.f)
	, iGameObject()
{
...
cAnimationComponent::~cAnimationComponent(void) {
	mapAnimationFriendlyNameTo_pScene.erase(mapAnimationFriendlyNameTo_pScene.begin(), mapAnimationFriendlyNameTo_pScene.end());
	m_mapBoneNameToBoneIndex.erase(m_mapBoneNameToBoneIndex.begin(), m_mapBoneNameToBoneIndex.end());
...
void cAnimationComponent::BoneTransform(float TimeInSeconds,
	...
	AnimaTime += AnimationTime;
	if (AnimaTime > TicksPerSecond) {
		AnimaTime = 0.f;
		AnimaQueue.pop();
	}

	if (AnimaQueue.empty()) {
		if (mapAnimationFriendlyNameTo_pScene.find("Idle") != mapAnimationFriendlyNameTo_pScene.end()) {
			AnimaQueue.push("Idle");
		}
		else return;
	}
...
bool cAnimationComponent::QueueAnimation(std::string AnimaName) {
	if (AnimaQueue.size() > 1) //Limiting buffered inputs!
		return false;
